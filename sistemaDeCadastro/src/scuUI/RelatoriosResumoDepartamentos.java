...
public class RelatoriosResumoDepartamentos extends JPanel {
	...
	static DefaultTableModel modelo = new DefaultTableModel(
			...
			new String[] {
				"Nome do Departamento", "Numero de Funcionarios", "Gasto Total"
			}
		){
	 ...
	 * Create the panel.
	 */
	public RelatoriosResumoDepartamentos() {
		...
		JLabel lblDescrição = new JLabel("Insira uma faixa de gasto, se desejar");
	...
	public static void setTableData(DataCare dc) {
		...
		for(int i = 0; i < dc.getDepartamentosArray().size(); i++) {
			modelo.addRow(new Object[] {dc.getDepartamentosByIndex(i).getNome(),
						  dc.getDepartamentosByIndex(i).concatTodosFuncionarios().size(),
						  dc.getDepartamentosByIndex(i).getGasto()});
		}
	}
	
	public void filtraData(DataCare dc, double gastoMin, double gastoMax) {
		modelo.setRowCount(0);
		for(int i = 0; i < dc.getDepartamentosArray().size(); i++) {
			double depGasto = dc.getDepartamentosByIndex(i).getGasto();
			if(depGasto >= gastoMin && depGasto <= gastoMax) {
				modelo.addRow(new Object[] {
						dc.getDepartamentosByIndex(i).getNome(),
						dc.getDepartamentosByIndex(i).concatTodosFuncionarios().size(),
						dc.getDepartamentosByIndex(i).getGasto()
				});
			}
